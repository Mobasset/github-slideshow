"""
CTR/GPEH/CTUM event parser.

Handles structured event files where each row/record describes a discrete
network event with IE fields. Supports:
  - CSV-style event exports (Ericsson OSS "event export" tool output)
  - Flat text event logs
  - Binary CTR (similar structure to GPEH but with different record layout)
...
# Abbreviation mapping for compact log formats
...
# Column aliases for CSV event exports
...
def _parse_ts(val: Any) -> int:
    """Convert various timestamp representations to epoch ms."""
    if val is None or (isinstance(val, float) and np.isnan(val)):
        return 0
    val_str = str(val).strip()
    # Already epoch ms (large integer)
    try:
        v = int(float(val_str))
...
def _normalize_event_type(raw: str) -> str:
    """Normalize raw event type string to canonical form."""
    raw = raw.strip().upper()
...
def parse_csv_events(data: bytes, source_file: str = "events_csv") -> pd.DataFrame:
    ...
    if "timestamp_ms" in df.columns:
        ...
        # Try to find any timestamp-like column
        for col in df.columns:
    ...
    if "cause_code" in df.columns:
        df["event_cause"] = df["cause_code"].astype(str)
    else:
        df["event_cause"] = ""
...
def parse_text_events(data: bytes, source_file: str = "events_log") -> pd.DataFrame:
    """Parse a free-text event log file."""
    text = data.decode("utf-8", errors="replace")
        ...
        records.append(
            {
                ...
                "rab_setup": int(
                    event_type in ("INTERNAL_RAB_ESTABLISHMENT", "INTERNAL_ERAB_SETUP")
                ),
...
def parse_events_bytes(data: bytes, filename: str = "events") -> pd.DataFrame:
    """Auto-detect event file format and parse accordingly."""
    # Detect format by content sniffing
    sample = data[:512]

    # Binary CTR/GPEH — delegate to gpeh_parser
    if sample[:4] == b"\x00\x00\x00\x04" or (
        len(sample) >= 4
        and sample[0] == 0x00
        and sample[1] == 0x00
        and sample[2] in (0x00, 0x01)
    ):
        from parsers.gpeh_parser import parse_gpeh_bytes
        return parse_gpeh_bytes(data, source_file=filename)

    # XML/gzip CTUM — delegate to ctum_parser
    if sample[:2] == b"\x1f\x8b" or sample[:5] in (b"<?xml", b"<meas"):
        from parsers.ctum_parser import parse_ctum_bytes
        return parse_ctum_bytes(data, source_file=filename)

    # CSV — check for commas/tabs in first line
    try:
